...
async function processCreateTPC(client, alice, bob, ab, bb, p1Sig) {
  ...
  .on('receipt', function(receipt){
      ...
      var msg = "agree create tpc,p2," + tmp_channel_id.toString();
      client.write(msg);
...
async function processMsg(client, msg) { //接收client发来的信息
    ...
    if (msg_arr[0] == 'create tpc') {
      ...
      if (p1Sig == p1Sig_check) {
        ...
        var p1Sig_check = await TPC_OBJ.generateSignatures(msgHash, p1);
        if (p1Sig == p1Sig_check) {
          const msgHash_2 = await web3.utils.soliditySha3(
            {t: 'string', v: txstr},
            {t: 'address', v: p2},
            {t: 'uint256', v: version}
          );
          var p2Sig = await TPC_OBJ.generateSignatures(msgHash_2, p2);
